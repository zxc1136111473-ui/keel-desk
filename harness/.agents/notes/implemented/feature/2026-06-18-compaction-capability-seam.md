...
- **Wiring**: `dsh-base` loads zero-config `dsh-token-meter`, `dsh-compaction-basic`, then `dsh-command-compact`; `dsh-tui` sits on that base, prints `contextPressure` occupancy at each REPL prompt, and maps `/compact` onto `ctx.compaction.compactNow()`. Service-wide defaults make the composition usable without repeated numeric policy.
